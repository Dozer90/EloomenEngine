...
namespace eloo::float4 {
    ...
    public:
        ...
        // CODE REVIEW: Missing implementation (same as float2/float3)
        // Declared but not implemented in float4.cpp - will cause linker errors
        values& operator + ();
        values& operator - ();
...
using float4_v = eloo::float4::values;