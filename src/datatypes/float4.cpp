...
bool operator != (const float4::values& lhs, const float4::values& rhs) {
    return
        lhs.x() != rhs.x() ||
        lhs.y() != rhs.y() ||
        lhs.z() != rhs.z() ||
...
bool operator != (const float4::values& lhs, float rhs) {
    return
        lhs.x() != rhs ||
        lhs.y() != rhs ||
        lhs.z() != rhs ||
...
float4::values operator - (const float4::values& lhs) {
    return {
        -lhs.x(),
        -lhs.y(),
        -lhs.z(),
...
float4::values operator / (const float4::values& lhs, const float4::values& rhs) {
    return {
        lhs.x() / rhs.x(),
        lhs.y() / rhs.y(),
        lhs.z() / rhs.z(),
...
float4::values operator / (const float4::values& lhs, float rhs) {
