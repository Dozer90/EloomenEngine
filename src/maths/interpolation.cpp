...
#define DEFINE_INTERPOLATION_TYPE(interp_t) FORCE_INLINE constexpr float blend_##interp_t(float t, eloo::math::interpolation::ease_type ease) { \
    switch (ease) { \
...
FORCE_INLINE constexpr float blend_##interp_t(float t, eloo::math::interpolation::ease_type ease, param_t param_name = param_default_value) { \
    switch (ease) { \
...
namespace {
    ...
    // Log          //////////////////////////////////////
    FORCE_INLINE constexpr float log_in(float t) {
        return eloo::math::log(1.0f + t * (eloo::math::f32::E - 1.0f));
    }

    FORCE_INLINE constexpr float log_out(float t) {
        return 1.0f - eloo::math::log(1.0f + (1.0f - t) * (eloo::math::f32::E - 1.0f));
    }

    FORCE_INLINE constexpr float log_in_out(float t) {
        t *= 2.0f;
    ...
    // Bezier       //////////////////////////////////////
    FORCE_INLINE constexpr float blend_bezier(float t, float ctrl1 = 0.1f, float ctrl2 = 0.1f) {
        const float omt = 1.0f - t;
        return 3.0f * omt * omt * t * ctrl1 + 3.0f * omt * t * t * ctrl2 + t * t * t;
    }

    // Step         //////////////////////////////////////
    FORCE_INLINE constexpr float blend_step(float t, unsigned int steps) {
        return round(t * steps) / static_cast<float>(steps);
    }

    // Spring       //////////////////////////////////////
    //https://www.desmos.com/calculator/jr8dd7xjr6
    FORCE_INLINE constexpr float blend_spring(float t, float frequency = 0.64f, float oscillation = 5.6f, float decay = 1.4f) {
...
float eloo::math::interpolation::interpolate(float from, float to, float t, blend_func type, blend_opt options) {
    ...
    switch (type) {
        ...
        case blend_func::poly: {
            ...
            ASSERT_FALSE("Attempting to call interpolation::interpolate() of blend_func::blend_poly without passing in a poly_opt struct.");
            break;
        }
        ...
        case blend_func::step: {
            ...
            ASSERT_FALSE("Attempting to call interpolation::interpolate() of blend_func::blend_steps without passing in a step_opt struct.");
            break;
        }
